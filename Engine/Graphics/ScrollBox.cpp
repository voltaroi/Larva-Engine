...
void ScrollBox::updateItemPositions()
{
    ...
    for (auto &item : items)
    {
        ...
        else if (item.type == Item::BUTTON && item.button)
        {
            ...
            // Update button position using setPosition
            float btnX = x + 10.0f;
            float btnY = currentY - item.button->height;
            item.button->manualPosition = true;
            item.button->setPosition(btnX, btnY, item.button->width, item.button->height);
...
void ScrollBox::setSize(float width, float height)
{
    this->width = width;
    this->height = height;
}